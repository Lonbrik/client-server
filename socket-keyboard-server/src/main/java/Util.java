...
public  class Util {
	...
	public static void addElement(ClientTask task,String elm) throws InterruptedException {
	...
	static void delay(ClientTask task, int millis) {
		if (millis > 0)
			try {
				Thread.sleep(millis);
	...
	static synchronized void log(ClientTask task, String mesg) {
		if (!LOG)
			 return;
	 ...
	 */
	static String circle(String[] request) {
		String x = request[1];
		String y = request[2];
